...
import { generateKeyPair, signPayload } from "crypto-core";
...
describe("SessionEngine", () => {
  it("authorizes session when threshold met and rejects duplicate approvals", async () => {
    ...
    await membership.applyMembershipEvent(promoted("u1", "e1"));
    await membership.applyMembershipEvent(promoted("u2", "e2"));
    await membership.applyMembershipEvent(promoted("u3", "e3"));

    const k1 = generateKeyPair();
    const k2 = generateKeyPair();
    await keyRegistry.registerKey("u1", k1.publicKey.export({ format: "der", type: "spki" }).toString("base64"));
    await keyRegistry.registerKey("u2", k2.publicKey.export({ format: "der", type: "spki" }).toString("base64"));

    const engine = new SessionEngine(new InMemorySessionRepository(), membership, keyRegistry, clock, 10);
    const session = await engine.createSession("rekey", "recovery_session_initiated", { plan: "x" });
    ...
    const afterOne = await engine.submitApproval(session.sessionId, "u1", sig1);
    expect(afterOne.status).toBe("pending");

    const afterTwo = await engine.submitApproval(session.sessionId, "u2", sig2);
    expect(afterTwo.status).toBe("authorized");

    await expect(engine.submitApproval(session.sessionId, "u2", sig2)).rejects.toThrow(/duplicate/);
  });

  it("rejects approvals after expiration", async () => {
    const membership = new MembershipRegistry(new InMemoryMembershipRepository(), { numerator: 2, denominator: 3 }, clock);
    const keyRegistry = new KeyRegistry(new InMemoryKeyRepository(), membership, clock);
    await membership.applyMembershipEvent({ eventId: "e1", eventType: "administrator_promoted", issuedAtMs: 1, nonce: "n", guildId: "g", payload: { discordUserId: "u1" }, payloadHash: "h", publicKey: "k", signature: "s" });
    const k1 = generateKeyPair();
    await keyRegistry.registerKey("u1", k1.publicKey.export({ format: "der", type: "spki" }).toString("base64"));
    ...
    await expect(engine.submitApproval(session.sessionId, "u1", sig)).rejects.toThrow(/expired/);
  });
});
